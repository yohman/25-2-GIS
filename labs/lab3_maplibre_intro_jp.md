...
## 🌐 ステップ6: GitHubにweek03をコミットしよう！

完成した `index.html` と `style.css` を、**Week 2で作成した同じGitHubリポジトリ**に追加して、Visual Studio Codeの画面からコミットしましょう。

やること：

1. Visual Studio Code でプロジェクトを開き、`week03` フォルダがリポジトリの中にあることを確認する
2. 左側の「ソース管理」アイコン（🔃）をクリックする
3. 変更されたファイル（`week03/index.html`、`week03/style.css`など）を確認する
4. ステージ（「+」ボタン）して、コミットする
5. コミットメッセージを書く（例：`Add week03 MapLibre project`）
6. 「✔」ボタンを押してコミットする
7. もし「Sync Changes（変更の同期）」ボタンが表示されたら、クリックして変更をGitHubに反映させる

✅ これで、week03の成果がリポジトリに安全に保存されました！

※ GitHub Pagesへの公開設定は次週行います。
